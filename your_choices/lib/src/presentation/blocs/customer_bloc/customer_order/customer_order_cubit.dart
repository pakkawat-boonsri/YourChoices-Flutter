...
class CustomerOrderCubit extends Cubit<CustomerOrderState> {
  ...
  final ReceiveCompletedOrderFromRestaurantUseCase receiveCompletedOrderFromRestaurantUseCase;
  CustomerOrderCubit({
    required this.receiveOrderFromRestaurantUseCase,
    required this.receiveCompletedOrderFromRestaurantUseCase,
  ...
  void receiveCompletedOrderFromRestaurant(String uid) {
    emit(CustomerOrderLoading());
    Future.delayed(const Duration(milliseconds: 800)).then((value) {
      try {
        final data = receiveCompletedOrderFromRestaurantUseCase.call(uid);
        data.listen((confirmOrderList) {
          emit(CustomerOrderLoaded(confirmOrderEntities: confirmOrderList));
        });
      } catch (e) {
