...
class _FilterOptionDetailViewState extends State<FilterOptionDetailView> {
  ...
  void initState() {
    ...
    context
        .read<AddNewAddOnsInFilterOptionDetailCubit>()
        .init(List<AddOnsEntity>.from(widget.filterOptionEntity.addOns ?? []));
    ...
    return Scaffold(
      ...
      appBar: CustomAppbar(
        ...
        onTap: () {
          ...
          context.read<AddNewAddOnsInFilterOptionDetailCubit>().onResetAddOns();
          context.read<AddNewAddOnsInFilterOptionDetailCubit>().resetDeleteAddOns();
        },
      ),
      body: Padding(
  ...
  Row _buildConfirmAndCancelButton(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        TouchableOpacity(
          onTap: () async {
            // TODO: update filter option
            if (_formKey.currentState!.validate()) {
              loadingDialog(context);
              List<AddOnsEntity> addOnsList = List.from(context.read<AddNewAddOnsInFilterOptionDetailCubit>().state);

              await context.read<FilterOptionCubit>().createFilterOption(
              ...
              if (mounted) {
                final currentDeleted =
                    List<AddOnsEntity>.from(context.read<AddNewAddOnsInFilterOptionDetailCubit>().deletedAddOns);
                // ignore: avoid_function_literals_in_foreach_calls
                currentDeleted.forEach((deleteAddOn) async {
                  await context.read<FilterOptionCubit>().onDeletingAddOnsInFilterOptionDetail(filterOptionEntity: filterOptionEntity, addOnsEntity: currentDeleted);
                });
              }

              Future.delayed(const Duration(seconds: 1)).then((value) {
                BlocProvider.of<AddNewAddOnsInFilterOptionDetailCubit>(context).onResetAddOns();
                BlocProvider.of<AddNewAddOnsInFilterOptionDetailCubit>(context).resetDeleteAddOns();
                Navigator.pop(context);
                Navigator.pop(context);
              });
            }
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      TouchableOpacity(
                        onTap: () async {
                          // TODO: add addon button
                          await context.read<AddNewAddOnsInFilterOptionDetailCubit>().onAddingAddOnInFilterOptionDetail(
                                AddOnsEntity(
  ...
  Widget _buildListOfAddOnsBloc() {
    return BlocBuilder<AddNewAddOnsInFilterOptionDetailCubit, List<AddOnsEntity>>(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        IconButton(
                          onPressed: () {
                            context.read<AddNewAddOnsInFilterOptionDetailCubit>().addingDeletedAddOns(addOnsEntity);
                            context
                                .read<AddNewAddOnsInFilterOptionDetailCubit>()
                                .onDeleteAddOnInFilterOptionDetail(addOnsEntity);
                          },
                          icon: Icon(
                            Icons.delete,
                            color: Colors.amber.shade900,
